...
async fn test_websocket_normal_request() {
    ...
    let url = MockWebSocketServer::new(responses).start(19099).await.unwrap();
...
async fn test_websocket_jsonrpc_error() {
    ...
    let url = MockWebSocketServer::new(responses).start(19100).await.unwrap();
    let ws_provider = WsWithTimeout::connect(url, None).await.unwrap();
    let provider = Provider::new(ws_provider);
...
async fn test_websocket_timeout() {
    ...
    let url = MockWebSocketServer::new(responses).start(19101).await.unwrap();
    let ws_provider = WsWithTimeout::connect_with_reconnects(url, 0, Some(Duration::from_millis(50)))
        .await
        .unwrap();
    let provider = Provider::new(ws_provider);
