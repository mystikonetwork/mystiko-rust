...
async fn test_provider_pool() {
    ...
    let mut pool = ProviderPool::builder()
        ...
        .build();
    assert!(pool.get_provider(56).await.is_none());
    assert!(!pool.has_provider(56).await);
    assert!(pool.check_provider(56).await.is_err());
    assert!(pool.get_or_create_provider(1).await.is_err());
    assert!(pool.get_or_create_provider(56).await.is_ok());
    assert!(pool.get_provider(56).await.is_some());
    assert!(pool.has_provider(56).await);
    assert!(pool.check_provider(56).await.is_ok());
    assert!(pool.get_or_create_provider(56).await.is_ok());
    assert!(pool.delete_provider(1).await.is_none());
    assert!(pool.delete_provider(56).await.is_some());
    pool.set_provider(1, Arc::new(create_test_provider())).await;
    assert!(pool.get_provider(1).await.is_some());
