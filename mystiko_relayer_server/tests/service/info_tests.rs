...
#[actix_rt::test]
async fn test_options_successful_0() {
...
#[actix_rt::test]
async fn test_options_successful_1() {
    // create mock provider
    let mock_provider = MockProvider::new();
    // create test server
    let mut server = TestServer::new(Some(mock_provider.clone())).await.unwrap();
...
#[actix_rt::test]
async fn test_options_successful_2() {
    // create mock provider
    let mock_provider = MockProvider::new();
    // create test server
    let mut server = TestServer::new(Some(mock_provider.clone())).await.unwrap();
...
#[actix_rt::test]
async fn minimum_gas_fee_chain_id_not_found() {
    let server = SERVER.get().await;
...
#[actix_rt::test]
async fn minimum_gas_fee_contract_config_not_found() {
    let server = SERVER.get().await;
    let result = minimum_gas_fee(
        server.app_state.relayer_config.as_ref(),
        5,
...
#[actix_rt::test]
async fn test_get_gas_price_error() {
    // create test server
    let server = SERVER.get().await;
    // init service
    let app = init_service(
...
#[actix_rt::test]
async fn test_get_minimum_gas_fee_error() {
    // create mock provider
    let mock_provider = MockProvider::new();
    // create test server
