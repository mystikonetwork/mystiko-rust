...
use mystiko_protos::core::synchronizer::v1::{ResetChainOptions, SynchronizerResetOptions};
...
async fn test_chain_reset() {
    ...
    let reset_options = SynchronizerResetOptions::default();
    let result = synchronizer.reset(reset_options).await;
    assert!(result.is_ok());

    let chain = ResetChainOptions::builder().chain_id(12345678_u64).build();
    let reset_options = SynchronizerResetOptions::builder().chains(vec![chain]).build();
    let result = synchronizer.reset(reset_options).await;
    assert!(result.is_ok());
}
