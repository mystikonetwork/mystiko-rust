...
#[tokio::test]
async fn test_handle_commitment_without_update() {
    ...
    for i in 0..6 {
        let data = build_queued_commitment_data(5, "0x4fd0ade06b9654437f46EA59e6edEe056F9d5EF7", i);
    ...
    for status in all_status {
        match status {
            ...
            | DepositStatus::Failed => {
                ...
                for i in 0..count {
                    let data = build_queued_commitment_data(5, "0x4fd0ade06b9654437f46EA59e6edEe056F9d5EF7", i + 1);
                    let result = handler.handle(&data, &option).await;
                    assert!(result.is_ok());
                    let updated_deposits = mystiko_db.deposits.find_all().await.unwrap();
                    db_deposits[i].data.status = DepositStatus::Queued as i32;
                    db_deposits[i].data.queued_transaction_hash = Some(String::from("0x5555"));
                    for j in 0..count {
...
fn build_queued_commitment_data(chain_id: u64, address: &str, count: usize) -> ChainData<FullData> {
    ...
    for i in 0..count {
        let commitment = Commitment {
            ...
            rollup_fee: None,
            encrypted_note: None,
            queued_transaction_hash: Some(decode_hex("0x5555").unwrap()),
