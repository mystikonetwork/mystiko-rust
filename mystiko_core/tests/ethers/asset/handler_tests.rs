...
#[tokio::test]
async fn test_erc20_approve_already_enough_allowance1() {
    let mut provider = MockProvider::new();
    let amount = U256::from(100100000000000000_u64);
    let amount_hex = format!("{:x}", amount);
    provider
        .expect_request()
        .withf(|method, params| {
        ...
        .returning(move |_, params| match params {
            JsonRpcParams::Value(params) => {
                let params = serde_json::to_string(&params).unwrap();
                if params.contains("0xdd62ed3e") {
                    Ok(serde_json::json!(U256::from(200100000000000000_u64).encode_hex()))
                } else {
                    Ok(serde_json::json!(amount.encode_hex()))
                }
            }
...
#[tokio::test]
async fn test_erc20_approve_already_enough_allowance2() {
    let mut provider = MockProvider::new();
    let amount = U256::from(100100000000000000_u64);
    let amount_hex = format!("{:x}", amount);
    provider
        ...
        .returning(move |_, params| match params {
            JsonRpcParams::Value(params) => {
                let params = serde_json::to_string(&params).unwrap();
                if params.contains("0xdd62ed3e") {
                    Ok(serde_json::json!(U256::from(100100000000000000_u64).encode_hex()))
                } else {
                    Ok(serde_json::json!(amount.encode_hex()))
                }
            }
