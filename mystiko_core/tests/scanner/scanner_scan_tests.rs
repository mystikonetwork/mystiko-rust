...
use mystiko_protos::core::scanner::v1::{ScannerScanOptions, ScannerScanResult};
...
async fn test_scan_default_options() {
    ...
    let option = ScannerScanOptions::builder().build();
    let result = scanner.scan(option).await;
    assert!(matches!(result.err().unwrap(), ScannerError::WalletHandlerError(_)));

    let option = ScannerScanOptions::builder().wallet_password("wrong_password").build();
    let result = scanner.scan(option).await;
    assert!(matches!(result.err().unwrap(), ScannerError::WalletHandlerError(_)));
    ...
    let option = ScannerScanOptions::builder()
        ...
        .build();
    let result = scanner.scan(option).await.unwrap();
    assert_eq!(result, ScannerScanResult::builder().build());
}
...
async fn test_scan_batch_with_owned_without_nullifier() {
    let (scanner, db, test_accounts) = create_scanner(1, None, HashMap::new(), None, None).await;
    let accounts = db.accounts.find_all().await.unwrap();
