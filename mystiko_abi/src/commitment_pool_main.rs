...
pub mod commitment_pool_main {
    ...
    #[doc = "CommitmentPoolMain was auto-generated with ethers-rs Abigen. More information at: https://github.com/gakonst/ethers-rs"]
    use std::sync::Arc;
    ...
    impl<M> Clone for CommitmentPoolMain<M> {
        fn clone(&self) -> Self {
            CommitmentPoolMain(self.0.clone())
        }
    }
    impl<M> std::ops::Deref for CommitmentPoolMain<M> {
        type Target = ethers::contract::Contract<M>;
        fn deref(&self) -> &Self::Target {
            &self.0
        }
    }
    impl<M> std::fmt::Debug for CommitmentPoolMain<M> {
    ...
    impl<M: ethers::providers::Middleware> CommitmentPoolMain<M> {
        ...
        pub fn new<T: Into<ethers::core::types::Address>>(
            ...
            ethers::contract::Contract::new(address.into(), COMMITMENTPOOLMAIN_ABI.clone(), client)
                .into()
        }
        ...
        pub fn events(&self) -> ethers::contract::builders::Event<M, CommitmentPoolMainEvents> {
    ...
    impl ethers::core::abi::AbiDecode for CommitmentPoolMainErrors {
        fn decode(
            ...
            {
                return Ok(CommitmentPoolMainErrors::AuditorIndexError(decoded));
            }
            if let Ok(decoded) =
                <AuditorNotesLengthError as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainErrors::AuditorNotesLengthError(decoded));
            }
            if let Ok(decoded) =
                <AuditorPublicKeyNotChanged as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainErrors::AuditorPublicKeyNotChanged(
                    decoded,
                ));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(CommitmentPoolMainErrors::VerifierUpdatesHasBeenDisabled(
                    decoded,
                ));
            }
    ...
    impl ethers::core::abi::AbiEncode for CommitmentPoolMainErrors {
        fn encode(self) -> Vec<u8> {
            match self {
                CommitmentPoolMainErrors::AuditorIndexError(element) => element.encode(),
                CommitmentPoolMainErrors::AuditorNotesLengthError(element) => element.encode(),
                CommitmentPoolMainErrors::AuditorPublicKeyNotChanged(element) => element.encode(),
                CommitmentPoolMainErrors::CommitmentHasBeenSubmitted(element) => element.encode(),
                CommitmentPoolMainErrors::Duplicated(element) => element.encode(),
                CommitmentPoolMainErrors::IndexOutOfBound(element) => element.encode(),
                CommitmentPoolMainErrors::Invalid(element) => element.encode(),
                CommitmentPoolMainErrors::NewRootIsDuplicated(element) => element.encode(),
                CommitmentPoolMainErrors::NotChanged(element) => element.encode(),
                CommitmentPoolMainErrors::NoteHasBeenSpent(element) => element.encode(),
                CommitmentPoolMainErrors::NumInputsGreaterThanZero(element) => element.encode(),
                CommitmentPoolMainErrors::OnlyOperator(element) => element.encode(),
                CommitmentPoolMainErrors::OnlyWhitelistedRoller(element) => element.encode(),
                CommitmentPoolMainErrors::OnlyWhitelistedSender(element) => element.encode(),
                CommitmentPoolMainErrors::OutputNotesLessThanThree(element) => element.encode(),
                CommitmentPoolMainErrors::RollupFeeToFew(element) => element.encode(),
                CommitmentPoolMainErrors::RollupSizeNotPowerOfTwo(element) => element.encode(),
                CommitmentPoolMainErrors::SanctionedAddress(element) => element.encode(),
                CommitmentPoolMainErrors::TreeHeightLessThanZero(element) => element.encode(),
                CommitmentPoolMainErrors::TreeHeightOutOfBounds(element) => element.encode(),
                CommitmentPoolMainErrors::TreeIsFull(element) => element.encode(),
                CommitmentPoolMainErrors::VerifierUpdatesHasBeenDisabled(element) => {
                    element.encode()
                }
            }
        }
    }
    impl ::std::fmt::Display for CommitmentPoolMainErrors {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                CommitmentPoolMainErrors::AuditorIndexError(element) => element.fmt(f),
                CommitmentPoolMainErrors::AuditorNotesLengthError(element) => element.fmt(f),
                CommitmentPoolMainErrors::AuditorPublicKeyNotChanged(element) => element.fmt(f),
                CommitmentPoolMainErrors::CommitmentHasBeenSubmitted(element) => element.fmt(f),
                CommitmentPoolMainErrors::Duplicated(element) => element.fmt(f),
                CommitmentPoolMainErrors::IndexOutOfBound(element) => element.fmt(f),
                CommitmentPoolMainErrors::Invalid(element) => element.fmt(f),
                CommitmentPoolMainErrors::NewRootIsDuplicated(element) => element.fmt(f),
                CommitmentPoolMainErrors::NotChanged(element) => element.fmt(f),
                CommitmentPoolMainErrors::NoteHasBeenSpent(element) => element.fmt(f),
                CommitmentPoolMainErrors::NumInputsGreaterThanZero(element) => element.fmt(f),
                CommitmentPoolMainErrors::OnlyOperator(element) => element.fmt(f),
                CommitmentPoolMainErrors::OnlyWhitelistedRoller(element) => element.fmt(f),
                CommitmentPoolMainErrors::OnlyWhitelistedSender(element) => element.fmt(f),
                CommitmentPoolMainErrors::OutputNotesLessThanThree(element) => element.fmt(f),
                CommitmentPoolMainErrors::RollupFeeToFew(element) => element.fmt(f),
                CommitmentPoolMainErrors::RollupSizeNotPowerOfTwo(element) => element.fmt(f),
                CommitmentPoolMainErrors::SanctionedAddress(element) => element.fmt(f),
                CommitmentPoolMainErrors::TreeHeightLessThanZero(element) => element.fmt(f),
                CommitmentPoolMainErrors::TreeHeightOutOfBounds(element) => element.fmt(f),
                CommitmentPoolMainErrors::TreeIsFull(element) => element.fmt(f),
                CommitmentPoolMainErrors::VerifierUpdatesHasBeenDisabled(element) => element.fmt(f),
            }
        }
    }
    impl ::std::convert::From<AuditorIndexError> for CommitmentPoolMainErrors {
        fn from(var: AuditorIndexError) -> Self {
            CommitmentPoolMainErrors::AuditorIndexError(var)
        }
    }
    impl ::std::convert::From<AuditorNotesLengthError> for CommitmentPoolMainErrors {
        fn from(var: AuditorNotesLengthError) -> Self {
            CommitmentPoolMainErrors::AuditorNotesLengthError(var)
        }
    }
    impl ::std::convert::From<AuditorPublicKeyNotChanged> for CommitmentPoolMainErrors {
        fn from(var: AuditorPublicKeyNotChanged) -> Self {
            CommitmentPoolMainErrors::AuditorPublicKeyNotChanged(var)
        }
    }
    impl ::std::convert::From<CommitmentHasBeenSubmitted> for CommitmentPoolMainErrors {
        fn from(var: CommitmentHasBeenSubmitted) -> Self {
            CommitmentPoolMainErrors::CommitmentHasBeenSubmitted(var)
        }
    }
    impl ::std::convert::From<Duplicated> for CommitmentPoolMainErrors {
        fn from(var: Duplicated) -> Self {
            CommitmentPoolMainErrors::Duplicated(var)
        }
    }
    impl ::std::convert::From<IndexOutOfBound> for CommitmentPoolMainErrors {
        fn from(var: IndexOutOfBound) -> Self {
            CommitmentPoolMainErrors::IndexOutOfBound(var)
        }
    }
    impl ::std::convert::From<Invalid> for CommitmentPoolMainErrors {
        fn from(var: Invalid) -> Self {
            CommitmentPoolMainErrors::Invalid(var)
        }
    }
    impl ::std::convert::From<NewRootIsDuplicated> for CommitmentPoolMainErrors {
        fn from(var: NewRootIsDuplicated) -> Self {
            CommitmentPoolMainErrors::NewRootIsDuplicated(var)
        }
    }
    impl ::std::convert::From<NotChanged> for CommitmentPoolMainErrors {
        fn from(var: NotChanged) -> Self {
            CommitmentPoolMainErrors::NotChanged(var)
        }
    }
    impl ::std::convert::From<NoteHasBeenSpent> for CommitmentPoolMainErrors {
        fn from(var: NoteHasBeenSpent) -> Self {
            CommitmentPoolMainErrors::NoteHasBeenSpent(var)
        }
    }
    impl ::std::convert::From<NumInputsGreaterThanZero> for CommitmentPoolMainErrors {
        fn from(var: NumInputsGreaterThanZero) -> Self {
            CommitmentPoolMainErrors::NumInputsGreaterThanZero(var)
        }
    }
    impl ::std::convert::From<OnlyOperator> for CommitmentPoolMainErrors {
        fn from(var: OnlyOperator) -> Self {
            CommitmentPoolMainErrors::OnlyOperator(var)
        }
    }
    impl ::std::convert::From<OnlyWhitelistedRoller> for CommitmentPoolMainErrors {
        fn from(var: OnlyWhitelistedRoller) -> Self {
            CommitmentPoolMainErrors::OnlyWhitelistedRoller(var)
        }
    }
    impl ::std::convert::From<OnlyWhitelistedSender> for CommitmentPoolMainErrors {
        fn from(var: OnlyWhitelistedSender) -> Self {
            CommitmentPoolMainErrors::OnlyWhitelistedSender(var)
        }
    }
    impl ::std::convert::From<OutputNotesLessThanThree> for CommitmentPoolMainErrors {
        fn from(var: OutputNotesLessThanThree) -> Self {
            CommitmentPoolMainErrors::OutputNotesLessThanThree(var)
        }
    }
    impl ::std::convert::From<RollupFeeToFew> for CommitmentPoolMainErrors {
        fn from(var: RollupFeeToFew) -> Self {
            CommitmentPoolMainErrors::RollupFeeToFew(var)
        }
    }
    impl ::std::convert::From<RollupSizeNotPowerOfTwo> for CommitmentPoolMainErrors {
        fn from(var: RollupSizeNotPowerOfTwo) -> Self {
            CommitmentPoolMainErrors::RollupSizeNotPowerOfTwo(var)
        }
    }
    impl ::std::convert::From<SanctionedAddress> for CommitmentPoolMainErrors {
        fn from(var: SanctionedAddress) -> Self {
            CommitmentPoolMainErrors::SanctionedAddress(var)
        }
    }
    impl ::std::convert::From<TreeHeightLessThanZero> for CommitmentPoolMainErrors {
        fn from(var: TreeHeightLessThanZero) -> Self {
            CommitmentPoolMainErrors::TreeHeightLessThanZero(var)
        }
    }
    impl ::std::convert::From<TreeHeightOutOfBounds> for CommitmentPoolMainErrors {
        fn from(var: TreeHeightOutOfBounds) -> Self {
            CommitmentPoolMainErrors::TreeHeightOutOfBounds(var)
        }
    }
    impl ::std::convert::From<TreeIsFull> for CommitmentPoolMainErrors {
        fn from(var: TreeIsFull) -> Self {
            CommitmentPoolMainErrors::TreeIsFull(var)
        }
    }
    impl ::std::convert::From<VerifierUpdatesHasBeenDisabled> for CommitmentPoolMainErrors {
        fn from(var: VerifierUpdatesHasBeenDisabled) -> Self {
            CommitmentPoolMainErrors::VerifierUpdatesHasBeenDisabled(var)
        }
    }
    #[derive(
        Clone,
    ...
    impl ::std::fmt::Display for CommitmentPoolMainEvents {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                CommitmentPoolMainEvents::AuditorPublicKeyFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::CommitmentIncludedFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::CommitmentQueuedFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::CommitmentSpentFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::EncryptedAuditorNoteFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::EncryptedAuditorNotesFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::OperatorChangedFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::RollupWhitelistDisabledFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::SanctionsCheckFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::SanctionsListFilter(element) => element.fmt(f),
                CommitmentPoolMainEvents::VerifierUpdateDisabledFilter(element) => element.fmt(f),
            }
        }
    }
    ...
    impl ethers::core::abi::AbiDecode for CommitmentPoolMainCalls {
        fn decode(
            ...
            {
                return Ok(CommitmentPoolMainCalls::GetCommitmentIncludedCount(decoded));
            }
            if let Ok(decoded) =
                <GetMinRollupFeeCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainCalls::GetMinRollupFee(decoded));
            }
            if let Ok(decoded) =
                <GetTreeCapacityCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainCalls::GetTreeCapacity(decoded));
            }
            if let Ok(decoded) =
                <IsHistoricCommitmentCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainCalls::IsHistoricCommitment(decoded));
            }
            if let Ok(decoded) =
                <IsKnownRootCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainCalls::IsKnownRoot(decoded));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(CommitmentPoolMainCalls::IsRollupWhitelistDisabled(decoded));
            }
            if let Ok(decoded) =
                <IsSpentSerialNumberCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainCalls::IsSpentSerialNumber(decoded));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(CommitmentPoolMainCalls::SetRollupWhitelistDisabled(decoded));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(CommitmentPoolMainCalls::SetVerifierUpdateDisabled(decoded));
            }
            if let Ok(decoded) =
                <TransactCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainCalls::Transact(decoded));
            }
            if let Ok(decoded) =
                <UpdateAuditorPublicKeyCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(CommitmentPoolMainCalls::UpdateAuditorPublicKey(decoded));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(CommitmentPoolMainCalls::UpdateSanctionsListAddress(decoded));
            }
            Err(ethers::core::abi::Error::InvalidData.into())
        }
    }
    impl ethers::core::abi::AbiEncode for CommitmentPoolMainCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                CommitmentPoolMainCalls::PathIndices(element) => element.encode(),
                CommitmentPoolMainCalls::AddEnqueueWhitelist(element) => element.encode(),
                CommitmentPoolMainCalls::AddRollupWhitelist(element) => element.encode(),
                CommitmentPoolMainCalls::AssetType(element) => element.encode(),
                CommitmentPoolMainCalls::AuditorCount(element) => element.encode(),
                CommitmentPoolMainCalls::ChangeOperator(element) => element.encode(),
                CommitmentPoolMainCalls::DisableRollupVerifier(element) => element.encode(),
                CommitmentPoolMainCalls::DisableSanctionsCheck(element) => element.encode(),
                CommitmentPoolMainCalls::DisableTransactVerifier(element) => element.encode(),
                CommitmentPoolMainCalls::EnableRollupVerifier(element) => element.encode(),
                CommitmentPoolMainCalls::EnableSanctionsCheck(element) => element.encode(),
                CommitmentPoolMainCalls::EnableTransactVerifier(element) => element.encode(),
                CommitmentPoolMainCalls::Enqueue(element) => element.encode(),
                CommitmentPoolMainCalls::GetAllAuditorPublicKeys(element) => element.encode(),
                CommitmentPoolMainCalls::GetAuditorPublicKey(element) => element.encode(),
                CommitmentPoolMainCalls::GetCommitmentIncludedCount(element) => element.encode(),
                CommitmentPoolMainCalls::GetMinRollupFee(element) => element.encode(),
                CommitmentPoolMainCalls::GetTreeCapacity(element) => element.encode(),
                CommitmentPoolMainCalls::IsHistoricCommitment(element) => element.encode(),
                CommitmentPoolMainCalls::IsKnownRoot(element) => element.encode(),
                CommitmentPoolMainCalls::IsRollupWhitelistDisabled(element) => element.encode(),
                CommitmentPoolMainCalls::IsSpentSerialNumber(element) => element.encode(),
                CommitmentPoolMainCalls::IsVerifierUpdateDisabled(element) => element.encode(),
                CommitmentPoolMainCalls::RemoveEnqueueWhitelist(element) => element.encode(),
                CommitmentPoolMainCalls::RemoveRollupWhitelist(element) => element.encode(),
                CommitmentPoolMainCalls::Rollup(element) => element.encode(),
                CommitmentPoolMainCalls::SanctionsCheck(element) => element.encode(),
                CommitmentPoolMainCalls::SanctionsList(element) => element.encode(),
                CommitmentPoolMainCalls::SetMinRollupFee(element) => element.encode(),
                CommitmentPoolMainCalls::SetRollupWhitelistDisabled(element) => element.encode(),
                CommitmentPoolMainCalls::SetVerifierUpdateDisabled(element) => element.encode(),
                CommitmentPoolMainCalls::Transact(element) => element.encode(),
                CommitmentPoolMainCalls::UpdateAuditorPublicKey(element) => element.encode(),
                CommitmentPoolMainCalls::UpdateSanctionsListAddress(element) => element.encode(),
            }
        }
    }
    impl ::std::fmt::Display for CommitmentPoolMainCalls {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                CommitmentPoolMainCalls::PathIndices(element) => element.fmt(f),
                CommitmentPoolMainCalls::AddEnqueueWhitelist(element) => element.fmt(f),
                CommitmentPoolMainCalls::AddRollupWhitelist(element) => element.fmt(f),
                CommitmentPoolMainCalls::AssetType(element) => element.fmt(f),
                CommitmentPoolMainCalls::AuditorCount(element) => element.fmt(f),
                CommitmentPoolMainCalls::ChangeOperator(element) => element.fmt(f),
                CommitmentPoolMainCalls::DisableRollupVerifier(element) => element.fmt(f),
                CommitmentPoolMainCalls::DisableSanctionsCheck(element) => element.fmt(f),
                CommitmentPoolMainCalls::DisableTransactVerifier(element) => element.fmt(f),
                CommitmentPoolMainCalls::EnableRollupVerifier(element) => element.fmt(f),
                CommitmentPoolMainCalls::EnableSanctionsCheck(element) => element.fmt(f),
                CommitmentPoolMainCalls::EnableTransactVerifier(element) => element.fmt(f),
                CommitmentPoolMainCalls::Enqueue(element) => element.fmt(f),
                CommitmentPoolMainCalls::GetAllAuditorPublicKeys(element) => element.fmt(f),
                CommitmentPoolMainCalls::GetAuditorPublicKey(element) => element.fmt(f),
                CommitmentPoolMainCalls::GetCommitmentIncludedCount(element) => element.fmt(f),
                CommitmentPoolMainCalls::GetMinRollupFee(element) => element.fmt(f),
                CommitmentPoolMainCalls::GetTreeCapacity(element) => element.fmt(f),
                CommitmentPoolMainCalls::IsHistoricCommitment(element) => element.fmt(f),
                CommitmentPoolMainCalls::IsKnownRoot(element) => element.fmt(f),
                CommitmentPoolMainCalls::IsRollupWhitelistDisabled(element) => element.fmt(f),
                CommitmentPoolMainCalls::IsSpentSerialNumber(element) => element.fmt(f),
                CommitmentPoolMainCalls::IsVerifierUpdateDisabled(element) => element.fmt(f),
                CommitmentPoolMainCalls::RemoveEnqueueWhitelist(element) => element.fmt(f),
                CommitmentPoolMainCalls::RemoveRollupWhitelist(element) => element.fmt(f),
                CommitmentPoolMainCalls::Rollup(element) => element.fmt(f),
                CommitmentPoolMainCalls::SanctionsCheck(element) => element.fmt(f),
                CommitmentPoolMainCalls::SanctionsList(element) => element.fmt(f),
                CommitmentPoolMainCalls::SetMinRollupFee(element) => element.fmt(f),
                CommitmentPoolMainCalls::SetRollupWhitelistDisabled(element) => element.fmt(f),
                CommitmentPoolMainCalls::SetVerifierUpdateDisabled(element) => element.fmt(f),
                CommitmentPoolMainCalls::Transact(element) => element.fmt(f),
                CommitmentPoolMainCalls::UpdateAuditorPublicKey(element) => element.fmt(f),
                CommitmentPoolMainCalls::UpdateSanctionsListAddress(element) => element.fmt(f),
            }
        }
    }
    impl ::std::convert::From<PathIndicesCall> for CommitmentPoolMainCalls {
        fn from(var: PathIndicesCall) -> Self {
            CommitmentPoolMainCalls::PathIndices(var)
        }
    }
    impl ::std::convert::From<AddEnqueueWhitelistCall> for CommitmentPoolMainCalls {
        fn from(var: AddEnqueueWhitelistCall) -> Self {
            CommitmentPoolMainCalls::AddEnqueueWhitelist(var)
        }
    }
    impl ::std::convert::From<AddRollupWhitelistCall> for CommitmentPoolMainCalls {
        fn from(var: AddRollupWhitelistCall) -> Self {
            CommitmentPoolMainCalls::AddRollupWhitelist(var)
        }
    }
    impl ::std::convert::From<AssetTypeCall> for CommitmentPoolMainCalls {
        fn from(var: AssetTypeCall) -> Self {
            CommitmentPoolMainCalls::AssetType(var)
        }
    }
    impl ::std::convert::From<AuditorCountCall> for CommitmentPoolMainCalls {
        fn from(var: AuditorCountCall) -> Self {
            CommitmentPoolMainCalls::AuditorCount(var)
        }
    }
    impl ::std::convert::From<ChangeOperatorCall> for CommitmentPoolMainCalls {
        fn from(var: ChangeOperatorCall) -> Self {
            CommitmentPoolMainCalls::ChangeOperator(var)
        }
    }
    impl ::std::convert::From<DisableRollupVerifierCall> for CommitmentPoolMainCalls {
        fn from(var: DisableRollupVerifierCall) -> Self {
            CommitmentPoolMainCalls::DisableRollupVerifier(var)
        }
    }
    impl ::std::convert::From<DisableSanctionsCheckCall> for CommitmentPoolMainCalls {
        fn from(var: DisableSanctionsCheckCall) -> Self {
            CommitmentPoolMainCalls::DisableSanctionsCheck(var)
        }
    }
    impl ::std::convert::From<DisableTransactVerifierCall> for CommitmentPoolMainCalls {
        fn from(var: DisableTransactVerifierCall) -> Self {
            CommitmentPoolMainCalls::DisableTransactVerifier(var)
        }
    }
    impl ::std::convert::From<EnableRollupVerifierCall> for CommitmentPoolMainCalls {
        fn from(var: EnableRollupVerifierCall) -> Self {
            CommitmentPoolMainCalls::EnableRollupVerifier(var)
        }
    }
    impl ::std::convert::From<EnableSanctionsCheckCall> for CommitmentPoolMainCalls {
        fn from(var: EnableSanctionsCheckCall) -> Self {
            CommitmentPoolMainCalls::EnableSanctionsCheck(var)
        }
    }
    impl ::std::convert::From<EnableTransactVerifierCall> for CommitmentPoolMainCalls {
        fn from(var: EnableTransactVerifierCall) -> Self {
            CommitmentPoolMainCalls::EnableTransactVerifier(var)
        }
    }
    impl ::std::convert::From<EnqueueCall> for CommitmentPoolMainCalls {
        fn from(var: EnqueueCall) -> Self {
            CommitmentPoolMainCalls::Enqueue(var)
        }
    }
    impl ::std::convert::From<GetAllAuditorPublicKeysCall> for CommitmentPoolMainCalls {
        fn from(var: GetAllAuditorPublicKeysCall) -> Self {
            CommitmentPoolMainCalls::GetAllAuditorPublicKeys(var)
        }
    }
    impl ::std::convert::From<GetAuditorPublicKeyCall> for CommitmentPoolMainCalls {
        fn from(var: GetAuditorPublicKeyCall) -> Self {
            CommitmentPoolMainCalls::GetAuditorPublicKey(var)
        }
    }
    impl ::std::convert::From<GetCommitmentIncludedCountCall> for CommitmentPoolMainCalls {
        fn from(var: GetCommitmentIncludedCountCall) -> Self {
            CommitmentPoolMainCalls::GetCommitmentIncludedCount(var)
        }
    }
    impl ::std::convert::From<GetMinRollupFeeCall> for CommitmentPoolMainCalls {
        fn from(var: GetMinRollupFeeCall) -> Self {
            CommitmentPoolMainCalls::GetMinRollupFee(var)
        }
    }
    impl ::std::convert::From<GetTreeCapacityCall> for CommitmentPoolMainCalls {
        fn from(var: GetTreeCapacityCall) -> Self {
            CommitmentPoolMainCalls::GetTreeCapacity(var)
        }
    }
    impl ::std::convert::From<IsHistoricCommitmentCall> for CommitmentPoolMainCalls {
        fn from(var: IsHistoricCommitmentCall) -> Self {
            CommitmentPoolMainCalls::IsHistoricCommitment(var)
        }
    }
    impl ::std::convert::From<IsKnownRootCall> for CommitmentPoolMainCalls {
        fn from(var: IsKnownRootCall) -> Self {
            CommitmentPoolMainCalls::IsKnownRoot(var)
        }
    }
    impl ::std::convert::From<IsRollupWhitelistDisabledCall> for CommitmentPoolMainCalls {
        fn from(var: IsRollupWhitelistDisabledCall) -> Self {
            CommitmentPoolMainCalls::IsRollupWhitelistDisabled(var)
        }
    }
    impl ::std::convert::From<IsSpentSerialNumberCall> for CommitmentPoolMainCalls {
        fn from(var: IsSpentSerialNumberCall) -> Self {
            CommitmentPoolMainCalls::IsSpentSerialNumber(var)
        }
    }
    impl ::std::convert::From<IsVerifierUpdateDisabledCall> for CommitmentPoolMainCalls {
        fn from(var: IsVerifierUpdateDisabledCall) -> Self {
            CommitmentPoolMainCalls::IsVerifierUpdateDisabled(var)
        }
    }
    impl ::std::convert::From<RemoveEnqueueWhitelistCall> for CommitmentPoolMainCalls {
        fn from(var: RemoveEnqueueWhitelistCall) -> Self {
            CommitmentPoolMainCalls::RemoveEnqueueWhitelist(var)
        }
    }
    impl ::std::convert::From<RemoveRollupWhitelistCall> for CommitmentPoolMainCalls {
        fn from(var: RemoveRollupWhitelistCall) -> Self {
            CommitmentPoolMainCalls::RemoveRollupWhitelist(var)
        }
    }
    impl ::std::convert::From<RollupCall> for CommitmentPoolMainCalls {
        fn from(var: RollupCall) -> Self {
            CommitmentPoolMainCalls::Rollup(var)
        }
    }
    impl ::std::convert::From<SanctionsCheckCall> for CommitmentPoolMainCalls {
        fn from(var: SanctionsCheckCall) -> Self {
            CommitmentPoolMainCalls::SanctionsCheck(var)
        }
    }
    impl ::std::convert::From<SanctionsListCall> for CommitmentPoolMainCalls {
        fn from(var: SanctionsListCall) -> Self {
            CommitmentPoolMainCalls::SanctionsList(var)
        }
    }
    impl ::std::convert::From<SetMinRollupFeeCall> for CommitmentPoolMainCalls {
        fn from(var: SetMinRollupFeeCall) -> Self {
            CommitmentPoolMainCalls::SetMinRollupFee(var)
        }
    }
    impl ::std::convert::From<SetRollupWhitelistDisabledCall> for CommitmentPoolMainCalls {
        fn from(var: SetRollupWhitelistDisabledCall) -> Self {
            CommitmentPoolMainCalls::SetRollupWhitelistDisabled(var)
        }
    }
    impl ::std::convert::From<SetVerifierUpdateDisabledCall> for CommitmentPoolMainCalls {
        fn from(var: SetVerifierUpdateDisabledCall) -> Self {
            CommitmentPoolMainCalls::SetVerifierUpdateDisabled(var)
        }
    }
    impl ::std::convert::From<TransactCall> for CommitmentPoolMainCalls {
        fn from(var: TransactCall) -> Self {
            CommitmentPoolMainCalls::Transact(var)
        }
    }
    impl ::std::convert::From<UpdateAuditorPublicKeyCall> for CommitmentPoolMainCalls {
        fn from(var: UpdateAuditorPublicKeyCall) -> Self {
            CommitmentPoolMainCalls::UpdateAuditorPublicKey(var)
        }
    }
    impl ::std::convert::From<UpdateSanctionsListAddressCall> for CommitmentPoolMainCalls {
        fn from(var: UpdateSanctionsListAddressCall) -> Self {
            CommitmentPoolMainCalls::UpdateSanctionsListAddress(var)
        }
    }
