...
use mystiko_relayer_config::types::{AssetType, CircuitType};
...
async fn test_create() {
    ...
    assert_eq!(
        config.find_gas_cost(AssetType::Main, CircuitType::Transaction1x0),
        500704
    );
    assert_eq!(
        config.find_gas_cost(AssetType::Main, CircuitType::Transaction1x1),
        619966
    );
    assert_eq!(
        config.find_gas_cost(AssetType::Main, CircuitType::Transaction1x2),
        705128
    );
    assert_eq!(
        config.find_gas_cost(AssetType::Erc20, CircuitType::Transaction2x0),
        598799
    );
    assert_eq!(
        config.find_gas_cost(AssetType::Erc20, CircuitType::Transaction2x1),
        708389
    );
    assert_eq!(
        config.find_gas_cost(AssetType::Erc20, CircuitType::Transaction2x2),
        803183
    );
}
