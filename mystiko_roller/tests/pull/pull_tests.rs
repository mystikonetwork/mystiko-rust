...
use mystiko_roller::pull::handle::PullHandle;
use serde_json::json;
...
pub async fn test_pull_from_provider() {
    ...
    assert_eq!(data.borrow().get_next_sync_block(), 0, "next sync block should be 0");
    ...
    let result = handle.pull().await;
    assert!(result.is_ok());
    assert_eq!(data.borrow().get_next_sync_block(), 256, "next sync block should be 0");
    assert_eq!(data.borrow().get_commitments_queue_count(), 1, "commitments mismatch");
    ...
    let result = handle.pull().await;
    assert!(result.is_ok());
    assert_eq!(data.borrow().get_next_sync_block(), 512, "next sync block should be 0");
    assert_eq!(
        data.borrow().get_commitments_queue_count(),
        logs.len(),
...
pub async fn test_pull_from_indexer() {
    ...
    assert!(result.is_ok());
    assert_eq!(
        data.borrow().get_next_sync_block(),
        block_number + 1,
    ...
    assert_eq!(data.borrow().get_included_count(), 0, "included count mismatch");
    std::mem::drop(server);
...
async fn create_mock_indexer_server(
    ...
    .await;
    let (cms, _) = cms.split_at(cm_count as usize);
...
async fn create_pull_handle(
...
) -> (PullHandle, Rc<RefCell<DataHandle>>, Arc<MockContext>) {
    ...
    let mut handle = PullHandle::new(pool_contract.address(), context_trait2, Rc::clone(&data_rc));
