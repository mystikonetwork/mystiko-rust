...
impl serde::Serialize for AssetSyncOptions {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("mystiko.core.scanner.v1.AssetSyncOptions", len)?;
        if !self.wallet_password.is_empty() {
            struct_ser.serialize_field("walletPassword", &self.wallet_password)?;
        }
        struct_ser.end()
...
impl<'de> serde::Deserialize<'de> for AssetSyncOptions {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = AssetSyncOptions;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct mystiko.core.scanner.v1.AssetSyncOptions")
            }

            fn visit_map<V>(self, mut map: V) -> std::result::Result<AssetSyncOptions, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut wallet_password__ = None;
        ...
        deserializer.deserialize_struct("mystiko.core.scanner.v1.AssetSyncOptions", FIELDS, GeneratedVisitor)
    }
}
