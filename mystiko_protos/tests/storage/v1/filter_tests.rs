use mystiko_protos::storage::v1::{
    column_value, ColumnValue, Condition, ConditionOperator, QueryFilter, SubFilter, SubFilterOperator,
};
...
fn test_equal_filter() {
    ...
    assert_eq!(
        equal_filter.values,
        vec![ColumnValue::builder().value(column_value::Value::I32Value(123)).build()]
    );
}
...
fn test_between_and_filter() {
    ...
    assert_eq!(
        ...
        vec![
            ColumnValue::builder().value(column_value::Value::I32Value(123)).build(),
            ColumnValue::builder().value(column_value::Value::I32Value(456)).build(),
        ]
    );
}
