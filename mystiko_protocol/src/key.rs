...
pub fn verification_secret_key(raw_secret_key: &[u8]) -> VerifySk {
...
pub fn verification_public_key(raw_secret_key: &[u8]) -> VerifyPk {
    assert_eq!(raw_secret_key.len(), VERIFY_SK_SIZE);
    let pk = PrivateKey::import(raw_secret_key.to_vec()).unwrap();
    let point = pk.public();
    fr_to_bytes(&point.x)
...
pub fn encryption_secret_key(raw_secret_key: &[u8]) -> EncSk {
    assert_eq!(raw_secret_key.len(), ENC_SK_SIZE);
    raw_secret_key.try_into().unwrap()
}

pub fn encryption_public_key(raw_secret_key: &[u8]) -> EncPk {
    assert_eq!(raw_secret_key.len(), ENC_SK_SIZE);
...
pub fn combined_secret_key(sk_verify: &[u8], sk_enc: &[u8]) -> FullSk {
...
pub fn combined_public_key(pk_verify: &[u8], pk_enc: &[u8]) -> FullPk {
