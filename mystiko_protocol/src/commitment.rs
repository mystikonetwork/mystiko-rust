...
impl Note {
    ...
    pub fn to_vec(&self) -> Vec<u8> {
        ...
        n.extend(&self.random_p);
        n.extend(&self.random_r);
        n.extend(&self.random_s);
        n.extend(bigint_to_32_bytes(&self.amount));
        n
    }
...
impl Commitment {
    pub fn new(
        ...
        let note = if let Some(n) = encrypted_note {
            decrypt_asymmetric(&n.sk_enc, &n.encrypted_note).map(Note::from_vec)
        } else {
