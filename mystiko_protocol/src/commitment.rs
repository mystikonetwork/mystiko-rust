...
pub fn encrypt_asymmetric(public_key: &[u8], plain_data: &[u8]) -> Vec<u8> {
    eccrypto::encrypt(public_key, plain_data)
}

pub fn decrypt_asymmetric(secret_key: &[u8], cipher_data: &[u8]) -> Vec<u8> {
...
mod tests {
    ...
    fn test_decrypt_asymmetric_compatible_with_js() {
        ...
        let dec_data = decrypt_asymmetric(&sk, enc_data_js.as_slice());
    ...
    fn test_random_encrypt_asymmetric() {
        ...
        let enc_data = encrypt_asymmetric(&pk, plain_text.as_slice());
        let dec_data = decrypt_asymmetric(&sk, enc_data.as_slice());
