...
async fn create_file_reader(file_path_str: &str) -> Result<BufReader<File>, FileError> {
    let file_path = Path::new(file_path_str);
    let file_handle = File::open(file_path)
        .await
        .map_err(|why| FileError::OpenFileError(file_path_str.parse().unwrap(), why.to_string()))?;
