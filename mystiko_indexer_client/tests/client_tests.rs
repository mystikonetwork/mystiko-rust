...
#[tokio::test]
async fn test_find_commitment_queued_for_contract() {
    let TestClientSetupData {
        mut mocked_server,
        indexer_client,
    } = setup().await.unwrap();
    ...
    let resp_list: Vec<CommitmentQueuedResponse> = vec![
        CommitmentQueuedResponse::builder()
            ...
            .block_num(100)
            .commit_hash("commit hash 1".to_string())
            .contract_address("contract_address1".to_string())
            .contract_id(1)
            .create_at(Some(100))
            .encrypted_note("encrypted_note 1".to_string())
            .leaf_index(1)
    ...
    let end_block = 10000;
    // test find without filter
    let address = String::from("contract_address1");
    let where_filter = CommitmentQueuedFilter::builder().build();
    let mocked_api_resp = ApiResponse {
        code: 0,
        result: &resp_list,
    ...
    let resp = indexer_client
        .find_commitment_queued_for_contract(
            &CommitmentQueuedForContractRequest::builder()
                .chain_id(chain_id)
                .address(address)
                .start_block(start_block)
                .end_block(end_block)
    ...
    let address = String::from("contract_address1");
    let where_filter = CommitmentQueuedFilter::builder()
        .commit_hash(String::from("commit hash 2"))
        .build();
    let mocked_api_resp = ApiResponse {
        code: 0,
    ...
    let resp = indexer_client
        ...
        .await;
    assert!(resp.is_err());
    assert!(resp.unwrap_err().downcast_ref::<std::io::Error>().is_none());
    m.assert_async().await;
}
...
#[tokio::test]
async fn test_find_commitment_included_for_contract() {
    let TestClientSetupData {
        mut mocked_server,
        indexer_client,
    } = setup().await.unwrap();
    ...
    let chain_id = 5;
    let start_block = 0;
    let end_block = 7580299;
    // test find without filter
    let address = String::from("0x4c77cc65b9011D7355Cae4e68978a613197Ae13c");
    let where_filter = CommitmentIncludedFilter::builder().build();
    let mocked_api_resp = ApiResponse {
        code: 0,
        result: &resp_list,
    ...
    let resp = indexer_client
        .find_commitment_included_for_contract(
            &CommitmentIncludedForContractRequest::builder()
                .chain_id(chain_id)
                .address(address)
                .start_block(start_block)
                .end_block(end_block)
...
#[tokio::test]
async fn test_find_commitment_spent_for_contract() {
    let TestClientSetupData {
        mut mocked_server,
        indexer_client,
    } = setup().await.unwrap();
    ...
    let chain_id = 5;
    let start_block = 0;
    let end_block = 7590302;
    // test find without filter
    let address = String::from("0xDDcF4fB58633a903b890f2C5ec11605063D8bFE1");
    let mocked_api_resp = ApiResponse {
        code: 0,
        result: &resp_list,
    };
    let m = mocked_server
        ...
        .await;
    let resp = indexer_client
        .find_commitment_spent_for_contract(
            &CommitmentSpentForContractRequest::builder()
                .chain_id(chain_id)
                .address(address)
                .start_block(start_block)
                .end_block(end_block)
