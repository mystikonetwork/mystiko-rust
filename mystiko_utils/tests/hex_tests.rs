...
fn test_encode_hex() {
    ...
    assert_eq!(
        encode_fixed_len_hex(&bytes, 16),
        format!("{}{}", hex_str, "00000000")
    );
    assert_eq!(encode_fixed_len_hex_with_prefix(&bytes, 0), "");
    assert_eq!(
        encode_fixed_len_hex_with_prefix(&bytes, 7),
        "0x01020304050607"
    );
    assert_eq!(
        encode_fixed_len_hex_with_prefix(&bytes, 8),
        format!("0x{}", hex_str)
    );
    assert_eq!(
...
fn test_decode_hex() {
    ...
    let hex_str = "0102030405060708";
    assert_eq!(
        decode_hex(format!("0x{}", hex_str).as_str()).unwrap(),
        bytes
    );
    assert_eq!(
        decode_hex(format!("0X{}", hex_str).as_str()).unwrap(),
    ...
    assert_eq!(decode_hex_with_length::<8>(hex_str).unwrap(), bytes);
    assert_eq!(
        decode_hex_with_length::<8>(format!("0x{}", hex_str).as_str()).unwrap(),
        bytes
    );
    assert_eq!(
        decode_hex_with_length::<8>(format!("0X{}", hex_str).as_str()).unwrap(),
    ...
    assert!(decode_hex_with_length::<8>("invalid_hex").is_err());
    assert!(decode_hex_with_length::<10>(hex_str).is_err());
}
