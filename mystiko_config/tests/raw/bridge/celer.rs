...
async fn default_config() -> RawCelerBridgeConfig {
    RawConfig::from_object::<RawCelerBridgeConfig>(RawCelerBridgeConfig::new(String::from(
        "Celer Bridge",
    )))
    .unwrap()
}
...
async fn test_import_valid_json_file() {
    let file_config =
        RawConfig::from_file::<RawCelerBridgeConfig>("tests/files/bridge/celer.valid.json")
            .await
            .unwrap();
...
async fn test_import_invalid_json_file() {
    let file_config =
        RawConfig::from_file::<RawCelerBridgeConfig>("tests/files/bridge/celer.invalid.json").await;
...
async fn test_import_valid_json_str() {
    ...
    let str_config = RawConfig::from_json_string::<RawCelerBridgeConfig>(json_str).unwrap();
