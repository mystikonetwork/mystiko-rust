...
async fn default_config() -> RawAxelarBridgeConfig {
    RawConfig::from_object::<RawAxelarBridgeConfig>(RawAxelarBridgeConfig::new(String::from(
        "Axelar Bridge",
    )))
    .unwrap()
}
...
lazy_static! {
    static ref CONFIG_CREATER: AsyncOnce<RawAxelarBridgeConfig> =
...
async fn test_import_valid_json_file() {
    let file_config =
        RawConfig::from_file::<RawAxelarBridgeConfig>("tests/files/bridge/axelar.valid.json")
            .await
            .unwrap();
...
async fn test_import_invalid_json_file() {
    let file_config =
        RawConfig::from_file::<RawAxelarBridgeConfig>("tests/files/bridge/axelar.invalid.json")
            .await;
...
async fn test_import_valid_json_str() {
    ...
    let str_config = RawConfig::from_json_string::<RawAxelarBridgeConfig>(json_str).unwrap();
