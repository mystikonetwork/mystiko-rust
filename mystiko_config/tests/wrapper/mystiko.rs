...
lazy_static! {
    static ref CONFIG_CREATER: AsyncOnce<MystikoConfig> = AsyncOnce::new(async {
       default_mystiko_config().await
    });
    static ref RAW_CONFIG_CREATER: AsyncOnce<RawMystikoConfig> = AsyncOnce::new(async {
       default_raw_config().await
    });
}
