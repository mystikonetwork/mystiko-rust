...
async fn default_config() -> PoolContractConfig {
    ...
    PoolContractConfig::new(raw_config.clone(), Some(
        AuxData::new(
...
lazy_static! {
    static ref CONFIG_CREATER: AsyncOnce<PoolContractConfig> = AsyncOnce::new(async {
        default_config().await
    });
    static ref RAW_CONFIG_CREATER: AsyncOnce<RawPoolContractConfig> = AsyncOnce::new(async {
...
async fn test_equality() {
    ...
    assert_eq!(config.asset_address().unwrap(), raw_config.asset_address.clone().unwrap());
