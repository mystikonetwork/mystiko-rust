...
async fn test_equality() {
    ...
    assert_eq!(config.asset_address().unwrap().unwrap(), "0xEC1d5CfB0bf18925aB722EeeBCB53Dc636834e8a".to_string());
    assert_eq!(
        config.recommended_amounts().unwrap().iter().map(|a| a.to_string()).collect::<Vec<String>>(),
        vec![
            String::from("10000000000000000"),
            String::from("100000000000000000"),
        ]
    );
    assert_eq!(config.recommended_amounts_number().unwrap(), vec![1f64, 10f64]);
    assert_eq!(config.min_rollup_fee().unwrap().to_string(), "40000000000000000".to_string());
    assert_eq!(config.min_rollup_fee_number().unwrap(), 4f64);

    let circuit_names =
        config.circuits().unwrap().iter().map(|conf| conf.name().clone()).collect::<Vec<String>>();
    assert_eq!(circuit_names.contains(&String::from("zokrates-2.0-rollup1")), true);
    assert_eq!(config.pool_contract().unwrap().address(), raw_config.pool_address);
...
async fn test_executor_fee_asset() {
    ...
    config = config.mutate(Some(raw_config.clone()), None).unwrap();
    assert_eq!(config.executor_fee_asset().unwrap(), config.asset().unwrap());
    ...
    assert_eq!(config.executor_fee_asset().unwrap(), main_asset_config().await);
...
async fn test_peer_contract() {
    ...
    assert_eq!(config.min_rollup_fee().unwrap().to_string(), "12345".to_string());
...
async fn test_invalid_raw_config_3() {
    ...
    assert_eq!(
        ...
        contract=0x961f315a836542e603a3df2e0dd9d4ecd06ebc67".to_string()
    );
}
