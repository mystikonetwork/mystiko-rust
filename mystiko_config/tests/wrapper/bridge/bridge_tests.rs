use mystiko_config::{
    create_raw_from_file, BridgeConfig, RawAxelarBridgeConfig, RawBridgeConfig, RawCelerBridgeConfig,
    RawLayerZeroBridgeConfig, RawPolyBridgeConfig, RawTBridgeConfig,
};
...
async fn test_to_proto() {
    ...
    let raw_tbridge_config = create_raw_from_file::<RawTBridgeConfig>(TBRIDGE_CONFIG_FILE)
        .await
        .unwrap();
    let bridge_config1 = BridgeConfig::new(Arc::new(RawBridgeConfig::Axelar(Arc::new(raw_axelar_config)))).to_proto();
    let bridge_config2 = BridgeConfig::new(Arc::new(RawBridgeConfig::Celer(Arc::new(raw_celer_config)))).to_proto();
    let bridge_config3 =
        BridgeConfig::new(Arc::new(RawBridgeConfig::LayerZero(Arc::new(raw_layer_zero_config)))).to_proto();
    let bridge_config4 = BridgeConfig::new(Arc::new(RawBridgeConfig::Poly(Arc::new(raw_poly_config)))).to_proto();
    let bridge_config5 = BridgeConfig::new(Arc::new(RawBridgeConfig::Tbridge(Arc::new(raw_tbridge_config)))).to_proto();
    assert_eq!(&bridge_config1.name, "Axelar Bridge");
    assert_eq!(&bridge_config2.name, "Celer Bridge");
    assert_eq!(&bridge_config3.name, "LayerZero Bridge");
    assert_eq!(&bridge_config4.name, "Poly Bridge");
    assert_eq!(&bridge_config5.name, "Mystiko Testnet Bridge");
    assert_eq!(
        bridge_config1.bridge_type(),
        mystiko_protos::common::v1::BridgeType::Axelar
    );
    assert_eq!(
        bridge_config2.bridge_type(),
        mystiko_protos::common::v1::BridgeType::Celer
    );
    assert_eq!(
        bridge_config3.bridge_type(),
        mystiko_protos::common::v1::BridgeType::LayerZero
    );
    assert_eq!(
        bridge_config4.bridge_type(),
        mystiko_protos::common::v1::BridgeType::Poly
    );
    assert_eq!(
        bridge_config5.bridge_type(),
        mystiko_protos::common::v1::BridgeType::Tbridge
    );
}
