...
lazy_static! {
    static ref CONFIG_CREATER: AsyncOnce<ChainConfig> = AsyncOnce::new(async {
        default_chain_config().await
    });
    static ref RAW_CONFIG_CREATER: AsyncOnce<RawChainConfig> = AsyncOnce::new(async {
        default_raw_config().await
    });
}
