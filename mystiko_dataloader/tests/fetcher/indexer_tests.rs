use log::LevelFilter;
use mockito::*;
use mystiko_config::MystikoConfig;
use mystiko_dataloader::data::FullData;
use mystiko_dataloader::data::LiteData;
use mystiko_dataloader::fetcher::{ContractFetchOptions, DataFetcher, FetchOptions, IndexerFetcher};
use mystiko_indexer_client::{DataLoaderRequest, IndexerClientBuilder};
use mystiko_utils::convert::biguint_str_to_bytes;
use mystiko_utils::hex::decode_hex;
use std::sync::Arc;

#[tokio::test]
async fn test_fulldata_fetch() {
    let _ = env_logger::builder()
        .filter_module("mystiko_dataloader", LevelFilter::Debug)
        .try_init();
    let mut mocked_server = Server::new_async().await;
    let mocked_server_url = mocked_server.url();
    let indexer_client = IndexerClientBuilder::new(&mocked_server_url).build().unwrap();
    let test_chain_id = 137;
    let test_address = "0xCB255075f38C75EAf2DE8A72897649dba9B90299";
    let mystiko_config = Arc::new(
        MystikoConfig::from_json_file("./tests/files/config/mystiko.json")
            .await
            .unwrap(),
    );
    let indexer_filter_size = mystiko_config.find_chain(test_chain_id).unwrap().indexer_filter_size();
    let test_start_block: u64 = 1100000;
    let test_end_block: u64 = test_start_block + indexer_filter_size - 1;
    let test_commitment_hash =
        "16729047340041860107670974487432292244530476292857805032774393627440911777131".to_string();
    let test_transaction_hash = "0xd699146e822e2826ebb3e0ecd4abae15f1fc07096bf6fdcd2af5705df0aff753".to_string();
    let test_nullifier = "12211436598431488691766057295382431251629992667648389012648413950694818133703".to_string();
    let mock_resp = serde_json::json!({
        "code": 0,
        "result": [
              {
                "contractAddress": test_address,
                "startBlock": test_start_block,
                "actualEndBlock": test_end_block-1,
                "errorMsg": null,
                "error": false,
                "commitments": [
                  {
                    "commitmentHash": test_commitment_hash,
                    "status": "succeeded",
                    "blockNumber": 1111111,
                    "includedBlockNumber": 1111111,
                    "srcChainBlockNumber": null,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": test_transaction_hash,
                    "srcChainTransactionHash": null
                  }
                ],
                "nullifiers": [
                  {
                    "nullifier": test_nullifier,
                    "blockNumber": 1122222,
                    "transactionHash": test_transaction_hash
                  }
                ]
              }
        ]
    });
    let path = format!("/chains/{}/full-data", test_chain_id);
    let request = vec![DataLoaderRequest::builder()
        .contract_address(test_address.to_string())
        .start_block(Some(test_start_block))
        .end_block(Some(test_end_block))
        .build()];
    let m = mocked_server
        .mock("post", path.as_str())
        .with_status(200)
        .match_query(Matcher::AllOf(vec![
            Matcher::UrlEncoded("startBlock".into(), test_start_block.to_string()),
            Matcher::UrlEncoded("endBlock".into(), test_end_block.to_string()),
        ]))
        .match_body(Matcher::JsonString(serde_json::to_string(&request).unwrap()))
        .with_body(serde_json::to_string(&mock_resp).unwrap())
        .with_header("content-type", "application/json")
        .create_async()
        .await;
    let indexer_fetcher: IndexerFetcher<FullData> = indexer_client.into();
    let contract_config = mystiko_config
        .find_contract_by_address(test_chain_id, test_address)
        .unwrap();
    let contract_fetch_option = vec![ContractFetchOptions::builder()
        .contract_config(contract_config.clone())
        .start_block(test_start_block)
        .target_block(test_end_block)
        .build()];
    let fetch_options = FetchOptions::builder()
        .chain_id(test_chain_id)
        .start_block(test_start_block)
        .target_block(test_end_block + 1)
        .config(Arc::clone(&mystiko_config))
        .contract_options(Some(contract_fetch_option))
        .build();
    let fetch_results1 = indexer_fetcher.fetch(&fetch_options).await;
    assert!(fetch_results1.is_ok());
    let result1 = fetch_results1.unwrap();
    assert_eq!(result1.chain_id, test_chain_id);
    assert_eq!(result1.contract_results.len(), 1);
    assert_eq!(result1.contract_results[0].address, test_address);
    assert_eq!(
        result1.contract_results[0].result.as_ref().unwrap().end_block,
        test_end_block - 1
    );
    assert_eq!(
        result1.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments[0]
            .commitment_hash,
        biguint_str_to_bytes(&test_commitment_hash).unwrap()
    );
    assert_eq!(
        result1.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments[0]
            .included_transaction_hash
            .as_ref()
            .unwrap(),
        decode_hex(&test_transaction_hash).as_ref().unwrap()
    );
    m.assert_async().await;

    //loop fetch
    let test_start_block2: u64 = test_end_block + 1;
    let test_end_block2: u64 = test_end_block + indexer_filter_size;
    let mock_resp = serde_json::json!({
        "code": 0,
        "result": [
              {
                "contractAddress": test_address,
                "startBlock": test_start_block2,
                "actualEndBlock": test_end_block2-1,
                "errorMsg": null,
                "error": false,
                "commitments": [
                  {
                    "commitmentHash": "16739047340041860107670974487432292244530476292857805032774393627440911777131",
                    "status": "succeeded",
                    "blockNumber": 1151111,
                    "includedBlockNumber": 1151111,
                    "srcChainBlockNumber": null,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": "0xd799146e822e2826ebb3e0ecd4abae15f1fc07096bf6fdcd2af5705df0aff753",
                    "srcChainTransactionHash": null
                  }
                ],
                "nullifiers": [
                  {
                    "nullifier": "12221436598431488691766057295382431251629992667648389012648413950694818133703",
                    "blockNumber": 1152222,
                    "transactionHash": "0xd799146e822e2826ebb3e0ecd4abae15f1fc07096bf6fdcd2af5705df0aff753"
                  }
                ]
              }
        ]
    });
    let request = vec![DataLoaderRequest::builder()
        .contract_address(test_address.to_string())
        .start_block(Some(test_start_block2))
        .end_block(Some(test_end_block2))
        .build()];
    let m = mocked_server
        .mock("post", path.as_str())
        .with_status(200)
        .match_query(Matcher::AllOf(vec![
            Matcher::UrlEncoded("startBlock".into(), test_start_block2.to_string()),
            Matcher::UrlEncoded("endBlock".into(), test_end_block2.to_string()),
        ]))
        .match_body(Matcher::JsonString(serde_json::to_string(&request).unwrap()))
        .with_body(serde_json::to_string(&mock_resp).unwrap())
        .with_header("content-type", "application/json")
        .create_async()
        .await;
    let contract_fetch_option = vec![ContractFetchOptions::builder()
        .contract_config(contract_config.clone())
        .start_block(test_start_block)
        .target_block(test_end_block2)
        .build()];
    let fetch_options = FetchOptions::builder()
        .chain_id(test_chain_id)
        .start_block(test_start_block)
        .target_block(test_end_block2 + 1)
        .config(Arc::clone(&mystiko_config))
        .contract_options(Some(contract_fetch_option))
        .build();
    let fetch_results1 = indexer_fetcher.fetch(&fetch_options).await;
    assert!(fetch_results1.is_ok());
    let result1 = fetch_results1.unwrap();
    assert_eq!(result1.chain_id, test_chain_id);
    assert_eq!(result1.contract_results.len(), 1);
    assert_eq!(result1.contract_results[0].address, test_address);
    assert_eq!(
        result1.contract_results[0].result.as_ref().unwrap().end_block,
        test_end_block2 - 1
    );
    assert_eq!(
        result1.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments
            .len(),
        2
    );
    assert_eq!(
        result1.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .nullifiers
            .len(),
        2
    );
    m.assert_async().await;
}
#[tokio::test]
async fn test_fulldata_fetch_all() {
    let _ = env_logger::builder()
        .filter_module("mystiko_dataloader", LevelFilter::Debug)
        .try_init();
    let mut mocked_server = Server::new_async().await;
    let mocked_server_url = mocked_server.url();
    let indexer_client = IndexerClientBuilder::new(&mocked_server_url).build().unwrap();
    let test_chain_id = 137;
    let test_address1 = "address1";
    let test_address2 = "address2";
    let mystiko_config = Arc::new(
        MystikoConfig::from_json_file("./tests/files/config/mystiko.json")
            .await
            .unwrap(),
    );
    let indexer_filter_size = mystiko_config.find_chain(test_chain_id).unwrap().indexer_filter_size();
    let test_start_block: u64 = 1100000;
    let test_end_block: u64 = test_start_block + indexer_filter_size - 1;
    let test_commitment_hash =
        "16729047340041860107670974487432292244530476292857805032774393627440911777131".to_string();
    let test_transaction_hash = "0xd699146e822e2826ebb3e0ecd4abae15f1fc07096bf6fdcd2af5705df0aff753".to_string();
    let test_nullifier = "12211436598431488691766057295382431251629992667648389012648413950694818133703".to_string();
    let mock_resp1 = serde_json::json!({
        "code": 0,
        "result": [
              {
                "contractAddress": test_address1,
                "startBlock": test_start_block,
                "actualEndBlock": test_end_block-1,
                "errorMsg": null,
                "error": false,
                "commitments": [
                  {
                    "commitmentHash": test_commitment_hash,
                    "status": "succeeded",
                    "blockNumber": 1111111,
                    "includedBlockNumber": 1111111,
                    "srcChainBlockNumber": null,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": test_transaction_hash,
                    "srcChainTransactionHash": null
                  }
                ],
                "nullifiers": [
                  {
                    "nullifier": test_nullifier,
                    "blockNumber": 1122222,
                    "transactionHash": test_transaction_hash
                  }
                ]
              },
              {
                "contractAddress": test_address2,
                "startBlock": test_start_block,
                "actualEndBlock": test_end_block-10,
                "errorMsg": null,
                "error": false,
                "commitments": [
                  {
                    "commitmentHash": test_commitment_hash,
                    "status": "succeeded",
                    "blockNumber": 1111112,
                    "includedBlockNumber": 1111112,
                    "srcChainBlockNumber": null,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": test_transaction_hash,
                    "srcChainTransactionHash": null
                  }
                ],
                "nullifiers": [
                  {
                    "nullifier": test_nullifier,
                    "blockNumber": 1122223,
                    "transactionHash": test_transaction_hash
                  }
                ]
              }
        ]
    });
    let path = format!("/chains/{}/full-data", test_chain_id);
    let m1 = mocked_server
        .mock("post", path.as_str())
        .with_status(200)
        .match_query(Matcher::AllOf(vec![
            Matcher::UrlEncoded("startBlock".into(), test_start_block.to_string()),
            Matcher::UrlEncoded("endBlock".into(), test_end_block.to_string()),
        ]))
        .with_body(serde_json::to_string(&mock_resp1).unwrap())
        .with_header("content-type", "application/json")
        .create_async()
        .await;
    let test_start_block2: u64 = test_end_block + 1;
    let test_end_block2: u64 = test_end_block + indexer_filter_size;
    let mock_resp2 = serde_json::json!({
        "code": 0,
        "result": [
              {
                "contractAddress": test_address1,
                "startBlock": test_start_block2,
                "actualEndBlock": test_end_block2-1,
                "errorMsg": null,
                "error": false,
                "commitments": [
                  {
                    "commitmentHash": "16739047340041860107670974487432292244530476292857805032774393627440911777131",
                    "status": "succeeded",
                    "blockNumber": 1151111,
                    "includedBlockNumber": 1151111,
                    "srcChainBlockNumber": null,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": "0xd799146e822e2826ebb3e0ecd4abae15f1fc07096bf6fdcd2af5705df0aff753",
                    "srcChainTransactionHash": null
                  }
                ],
                "nullifiers": [
                  {
                    "nullifier": "12221436598431488691766057295382431251629992667648389012648413950694818133703",
                    "blockNumber": 1152222,
                    "transactionHash": "0xd799146e822e2826ebb3e0ecd4abae15f1fc07096bf6fdcd2af5705df0aff753"
                  }
                ]
              },{
                "contractAddress": test_address2,
                "startBlock": test_start_block2,
                "actualEndBlock": test_end_block2-10,
                "errorMsg": null,
                "error": false,
                "commitments": [
                  {
                    "commitmentHash": "16749047340041860107670974487432292244530476292857805032774393627440911777131",
                    "status": "succeeded",
                    "blockNumber": 1151112,
                    "includedBlockNumber": 1151112,
                    "srcChainBlockNumber": null,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": "0xd899146e822e2826ebb3e0ecd4abae15f1fc07096bf6fdcd2af5705df0aff753",
                    "srcChainTransactionHash": null
                  }
                ],
                "nullifiers": [
                  {
                    "nullifier": "12231436598431488691766057295382431251629992667648389012648413950694818133703",
                    "blockNumber": 1152223,
                    "transactionHash": "0xd899146e822e2826ebb3e0ecd4abae15f1fc07096bf6fdcd2af5705df0aff753"
                  }
                ]
              }
        ]
    });

    let m2 = mocked_server
        .mock("post", path.as_str())
        .with_status(200)
        .match_query(Matcher::AllOf(vec![
            Matcher::UrlEncoded("startBlock".into(), test_start_block2.to_string()),
            Matcher::UrlEncoded("endBlock".into(), test_end_block2.to_string()),
        ]))
        .with_body(serde_json::to_string(&mock_resp2).unwrap())
        .with_header("content-type", "application/json")
        .create_async()
        .await;
    let fetch_options = FetchOptions::builder()
        .chain_id(test_chain_id)
        .start_block(test_start_block)
        .target_block(test_end_block2)
        .config(Arc::clone(&mystiko_config))
        .contract_options(None)
        .build();
    let indexer_fetcher: IndexerFetcher<FullData> = indexer_client.into();
    let fetch_results = indexer_fetcher.fetch(&fetch_options).await;

    assert!(fetch_results.is_ok());
    let result = fetch_results.unwrap();
    assert_eq!(result.chain_id, test_chain_id);
    assert_eq!(result.contract_results.len(), 2);
    assert_eq!(
        result.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments
            .len(),
        2
    );
    assert_eq!(
        result.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .nullifiers
            .len(),
        2
    );
    assert_eq!(
        result.contract_results[1]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments
            .len(),
        2
    );
    assert_eq!(
        result.contract_results[1]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .nullifiers
            .len(),
        2
    );
    m1.assert_async().await;
    m2.assert_async().await;
}
#[tokio::test]
async fn test_fulldata_fetch_error() {
    let _ = env_logger::builder()
        .filter_module("mystiko_dataloader", LevelFilter::Debug)
        .try_init();
    let mut mocked_server = Server::new_async().await;
    let mocked_server_url = mocked_server.url();
    let indexer_client = IndexerClientBuilder::new(&mocked_server_url).build().unwrap();
    let test_chain_id = 137;
    let test_address = "0xCB255075f38C75EAf2DE8A72897649dba9B90299";
    // fetch with response error
    let block_error_msg = String::from("Unknown error of contract.");
    let mystiko_config = Arc::new(
        MystikoConfig::from_json_file("./tests/files/config/mystiko.json")
            .await
            .unwrap(),
    );
    let indexer_filter_size = mystiko_config.find_chain(test_chain_id).unwrap().indexer_filter_size();
    let test_start_block: u64 = 1000000;
    let test_end_block: u64 = test_start_block + indexer_filter_size - 1;
    let request = vec![DataLoaderRequest::builder()
        .contract_address(test_address.to_string())
        .start_block(Some(test_start_block))
        .end_block(Some(test_end_block))
        .build()];
    let mock_resp = serde_json::json!({
        "code": 0,
        "result": [
            {
              "contractAddress": test_address.to_string(),
              "startBlock": test_start_block,
              "actualEndBlock": test_end_block,
              "commitments": [],
              "nullifiers": [],
              "errorMsg": block_error_msg,
              "error": true
            },
        ]
    });
    let path = format!("/chains/{}/full-data", test_chain_id);
    let m = mocked_server
        .mock("post", path.as_str())
        .with_status(200)
        .match_query(Matcher::AllOf(vec![
            Matcher::UrlEncoded("startBlock".into(), test_start_block.to_string()),
            Matcher::UrlEncoded("endBlock".into(), test_end_block.to_string()),
        ]))
        .match_body(Matcher::JsonString(serde_json::to_string(&request).unwrap()))
        .with_body(serde_json::to_string(&mock_resp).unwrap())
        .with_header("content-type", "application/json")
        .create_async()
        .await;
    let indexer_fetcher: IndexerFetcher<FullData> = indexer_client.into();
    let contract_config = mystiko_config
        .find_contract_by_address(test_chain_id, test_address)
        .unwrap();
    let contract_fetch_option2 = vec![ContractFetchOptions::builder()
        .contract_config(contract_config.clone())
        .start_block(test_start_block)
        .target_block(test_end_block)
        .build()];
    let fetch_options2 = FetchOptions::builder()
        .chain_id(test_chain_id)
        .start_block(test_start_block)
        .target_block(test_end_block)
        .config(Arc::clone(&mystiko_config))
        .contract_options(Some(contract_fetch_option2))
        .build();
    let fetch_results2 = indexer_fetcher.fetch(&fetch_options2).await;
    let result2 = fetch_results2.unwrap();
    assert_eq!(result2.contract_results.len(), 1);
    assert!(result2.contract_results[0].result.is_err());
    assert_eq!(
        result2.contract_results[0].result.as_ref().err().unwrap().to_string(),
        format!("fetcher contract with error {}", block_error_msg)
    );
    m.assert_async().await;
}

#[tokio::test]
async fn test_litedata_fetch() {
    let mut mocked_server = Server::new_async().await;
    let mocked_server_url = mocked_server.url();
    let indexer_client = IndexerClientBuilder::new(&mocked_server_url).build().unwrap();
    let test_chain_id = 137;
    let mystiko_config = Arc::new(
        MystikoConfig::from_json_file("./tests/files/config/mystiko.json")
            .await
            .unwrap(),
    );
    let indexer_filter_size = mystiko_config.find_chain(test_chain_id).unwrap().indexer_filter_size();
    let test_start_block: u64 = 1000000;
    let test_end_block: u64 = test_start_block + indexer_filter_size - 1;
    let test_address1 = "0xCB255075f38C75EAf2DE8A72897649dba9B90299".to_string();
    let test_address2 = "0x433dD8dBb9e631fbA277c91D09133Ae2616833Fa".to_string();
    let test_commitment_hash =
        "7018260368219958685591690287020455882335116439695048193945341664494747090540".to_string();
    let test_transaction_hash = "0xdde18155633283b4e45fa9b8db3add12b0ca4172c21b33eacceee5b41295fe2b".to_string();
    let test_encrypted_note = "".to_string();
    let mock_resp1 = serde_json::json!({
        "code": 0,
        "result": [{
            "contractAddress": test_address1,
            "startBlock": test_start_block,
            "actualEndBlock": test_end_block-1,
            "errorMsg": null,
            "error": false,
            "commitments": [
                {
                    "commitmentHash": test_commitment_hash,
                    "status": "srcSucceeded",
                    "blockNumber": 1046666,
                    "includedBlockNumber": null,
                    "srcChainBlockNumber": 1046666,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": test_transaction_hash,
                    "includedTransactionHash": null,
                    "srcChainTransactionHash": null
                },
                {
                    "commitmentHash": test_commitment_hash,
                    "status": "Queued",
                    "blockNumber": 1048888,
                    "includedBlockNumber": null,
                    "srcChainBlockNumber": null,
                    "leafIndex": 0,
                    "rollupFee": "200000000000000000",
                    "encryptedNote": test_encrypted_note,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": test_transaction_hash,
                    "srcChainTransactionHash": null
                },
                {
                    "commitmentHash": test_commitment_hash,
                    "status": "succeeded",
                    "blockNumber": 1047777,
                    "includedBlockNumber": 1047777,
                    "srcChainBlockNumber": null,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": null,
                    "srcChainTransactionHash": test_transaction_hash
                }
            ]
        }]
    });
    let path = format!("/chains/{}/lite-data", test_chain_id);
    let request1 = vec![DataLoaderRequest::builder()
        .contract_address(test_address1.to_string())
        .start_block(Some(test_start_block))
        .end_block(Some(test_end_block))
        .build()];
    let m1 = mocked_server
        .mock("post", path.as_str())
        .with_status(200)
        .match_query(Matcher::AllOf(vec![
            Matcher::UrlEncoded("startBlock".into(), test_start_block.to_string()),
            Matcher::UrlEncoded("endBlock".into(), test_end_block.to_string()),
        ]))
        .match_body(Matcher::JsonString(serde_json::to_string(&request1).unwrap()))
        .with_body(serde_json::to_string(&mock_resp1).unwrap())
        .with_header("content-type", "application/json")
        .create_async()
        .await;
    let mock_resp2 = serde_json::json!({
        "code": 0,
        "result": [{
            "contractAddress": test_address2,
            "startBlock": test_start_block,
            "actualEndBlock": test_end_block-10,
            "errorMsg": null,
            "error": false,
            "commitments": [
                {
                    "commitmentHash": "test_commitment_hash",
                    "status": "srcSucceeded",
                    "blockNumber": 1046666,
                    "includedBlockNumber": null,
                    "srcChainBlockNumber": 1046666,
                    "leafIndex": null,
                    "rollupFee": null,
                    "encryptedNote": null,
                    "queuedTransactionHash": null,
                    "includedTransactionHash": null,
                    "srcChainTransactionHash": "srcChainTransactionHash"
                },
                {
                    "commitmentHash": "test_commitment_hash2",
                    "status": "Queued",
                    "blockNumber": 1048888,
                    "includedBlockNumber": null,
                    "srcChainBlockNumber": null,
                    "leafIndex": 0,
                    "rollupFee": "200000000000000000",
                    "encryptedNote": "encryptedNote",
                    "queuedTransactionHash": "queuedTransactionHash",
                    "includedTransactionHash": null,
                    "srcChainTransactionHash": null
                }
            ]
        }]
    });
    let request1 = vec![DataLoaderRequest::builder()
        .contract_address(test_address2.to_string())
        .start_block(Some(test_start_block))
        .end_block(Some(test_end_block))
        .build()];
    let m2 = mocked_server
        .mock("post", path.as_str())
        .with_status(200)
        .match_query(Matcher::AllOf(vec![
            Matcher::UrlEncoded("startBlock".into(), test_start_block.to_string()),
            Matcher::UrlEncoded("endBlock".into(), test_end_block.to_string()),
        ]))
        .match_body(Matcher::JsonString(serde_json::to_string(&request1).unwrap()))
        .with_body(serde_json::to_string(&mock_resp2).unwrap())
        .with_header("content-type", "application/json")
        .create_async()
        .await;
    let indexer_fetcher: IndexerFetcher<LiteData> = indexer_client.into();

    let contract_config = mystiko_config
        .find_contract_by_address(test_chain_id, &test_address1)
        .unwrap();
    let contract_config2 = mystiko_config
        .find_contract_by_address(test_chain_id, &test_address2)
        .unwrap();
    let contract_fetch_option = vec![
        ContractFetchOptions::builder()
            .contract_config(contract_config.clone())
            .start_block(test_start_block)
            .target_block(test_end_block)
            .build(),
        ContractFetchOptions::builder()
            .contract_config(contract_config2.clone())
            .start_block(test_start_block)
            .target_block(test_end_block)
            .build(),
    ];
    let fetch_options = FetchOptions::builder()
        .chain_id(test_chain_id)
        .start_block(test_start_block)
        .target_block(test_end_block)
        .config(Arc::clone(&mystiko_config))
        .contract_options(Some(contract_fetch_option))
        .build();
    let fetch_results1 = indexer_fetcher.fetch(&fetch_options).await;
    assert!(fetch_results1.is_ok());
    let result1 = fetch_results1.unwrap();
    assert_eq!(result1.chain_id, test_chain_id);
    assert_eq!(result1.contract_results.len(), 2);
    assert_eq!(result1.contract_results[0].address, test_address1.to_string());
    assert_eq!(
        result1.contract_results[0].result.as_ref().unwrap().end_block,
        test_end_block - 1
    );
    assert_eq!(
        result1.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments
            .len(),
        3
    );
    assert_eq!(
        result1.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments[2]
            .encrypted_note
            .as_ref()
            .unwrap(),
        decode_hex(&test_encrypted_note).as_ref().unwrap()
    );
    assert_eq!(
        result1.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments[1]
            .block_number,
        1047777
    );
    assert_eq!(
        result1.contract_results[0]
            .result
            .as_ref()
            .unwrap()
            .data
            .as_ref()
            .unwrap()
            .commitments[2]
            .rollup_fee
            .as_ref()
            .unwrap(),
        biguint_str_to_bytes("200000000000000000").as_ref().unwrap()
    );
    assert!(result1.contract_results[1].result.is_err());
    m1.assert_async().await;
    m2.assert_async().await;
}
