...
use mystiko_dataloader::fetcher::{
    indexer::{IndexerFetcher, IndexerFetcherOptions},
    types::DataFetcher,
};
...
async fn test_fulldata_fetch() {
    ...
    let m = mocked_server
        ...
        .create_async()
        .await;
    let indexer_fetcher: IndexerFetcher<FullData> = IndexerFetcher::<FullData>::new(
        IndexerFetcherOptions::builder()
...
async fn test_litedata_fetch() {
    ...
    let m = mocked_server
        ...
        .create_async()
        .await;
    let indexer_fetcher: IndexerFetcher<LiteData> = IndexerFetcher::<LiteData>::new(
        IndexerFetcherOptions::builder()
            .chain_id(test_chain_id)
            .indexer_client(indexer_client)
            .build(),
...
async fn test_error_fetch() {
    ...
    assert_eq!(
        ...
        "chain_id param in ChainFetchOption(137) is different from IndexerFetcher(1)"
    );
    // FetchOption::Contracts
    let contracts_options1 = vec![ContractFetchOption::builder()
        .address(test_address.to_string())
        .chain_id(test_chain_id2)
        .config(Arc::clone(&mystiko_config))
        .start_block(test_start_block)
