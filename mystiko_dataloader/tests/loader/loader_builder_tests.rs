...
use mystiko_dataloader::loader::{AsyncFrom, ChainDataLoader, DataLoader, LoaderConfigOptions};
use mystiko_dataloader::DataLoaderError;
use mystiko_ethers::Providers;
use mystiko_protos::config::v1::ConfigOptions;
use mystiko_protos::loader::v1::{
    FetcherConfig, LoaderConfig, ProviderFetcherChainConfig, ProviderFetcherConfig, RuleValidatorCheckerType,
    RuleValidatorConfig, ValidatorConfig, ValidatorType,
};
    ...
    let cfg = LoaderConfig::builder()
        ...
        .fetcher_config(
            FetcherConfig::builder()
                .provider(ProviderFetcherConfig::builder().timeout_ms(1).build())
                .build(),
        )
        .build();
    let handler = MockHandler::new();
