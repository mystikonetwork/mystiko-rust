...
async fn test_empty_commitment() {
    let (validator, _handler, _mock, _, _) = create_full_data_validator(Some(vec![RuleCheckerType::Partial]));
    ...
    let contract_data = ContractData::builder()
        ...
        .start_block(1_u64)
        .end_block(100_u64)
        .data(FullData::builder().commitments(vec![cm]).nullifiers(vec![]).build())
        .build();
