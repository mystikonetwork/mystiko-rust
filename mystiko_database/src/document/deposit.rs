...
pub static DEPOSIT_SCHEMA: DocumentSchema = DocumentSchema {
    ...
    migrations: &["CREATE TABLE `deposits` (\
            ...
            `shielded_recipient_address` VARCHAR(255) NOT NULL,\
            `status` VARCHAR(32) NOT NULL,\
            `error_message` TEXT,\
            `wallet_id` VARCHAR(64) NOT NULL,\
            `dst_chain_id` INT NOT NULL,\
            `dst_chain_contract_address` VARCHAR(255) NOT NULL,\
            `dst_pool_address` VARCHAR(255) NOT NULL,\
            `asset_approve_transaction_hash` VARCHAR(255),\
            `transaction_hash` VARCHAR(255),\
            `relay_transaction_hash` VARCHAR(255),\
            `rollup_transaction_hash` VARCHAR(255))"],
    field_names: &[
        "chain_id",
        "contract_address",
        "pool_address",
