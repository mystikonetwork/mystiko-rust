...
impl<F: StatementFormatter, R: DocumentRawData, S: Storage<R>> NullifierCollection<F, R, S> {
    ...
    pub async fn insert(&self, nullifier: &Nullifier) -> Result<Document<Nullifier>, Error> {
    ...
    ) -> Result<Vec<Document<Nullifier>>, Error> {
        let mut collection = self.collection.lock().await;
        collection.insert_batch(nullifiers).await
    }

    pub async fn find(&self, filter: QueryFilter) -> Result<Vec<Document<Nullifier>>, Error> {
        let mut collection = self.collection.lock().await;
        collection.find::<Nullifier>(Some(filter)).await
    }

    pub async fn find_all(&self) -> Result<Vec<Document<Nullifier>>, Error> {
        let mut collection = self.collection.lock().await;
    ...
    pub async fn find_one(
        &self,
        filter: QueryFilter,
    ) -> Result<Option<Document<Nullifier>>, Error> {
        let mut collection = self.collection.lock().await;
        collection.find_one(Some(filter)).await
    }

    pub async fn find_by_id(&self, id: &str) -> Result<Option<Document<Nullifier>>, Error> {
        let mut collection = self.collection.lock().await;
    ...
    pub async fn update(
        &self,
        nullifier: &Document<Nullifier>,
    ) -> Result<Document<Nullifier>, Error> {
        let mut collection = self.collection.lock().await;
    ...
    pub async fn delete(&self, nullifier: &Document<Nullifier>) -> Result<(), Error> {
        let mut collection = self.collection.lock().await;
        collection.delete(nullifier).await
    }

    pub async fn delete_batch(&self, nullifiers: &Vec<Document<Nullifier>>) -> Result<(), Error> {
        let mut collection = self.collection.lock().await;
